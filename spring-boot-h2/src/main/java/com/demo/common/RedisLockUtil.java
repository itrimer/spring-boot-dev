...
public class RedisLockUtil {
    ...
    public RedisLockUtil(final String lockKey) {
    ...
    public RedisLockUtil(final String lockKey, final int timeoutMsecs) {
        this(lockKey);
        this.timeoutMsecs = timeoutMsecs;
    }

    public RedisLockUtil(final String lockKey, final int timeoutMsecs, final int expireMsecs) {
    ...
    public synchronized Boolean lock() throws InterruptedException {
        ...
        while (timeout >= 0) {
            ...
            final String currentValueStr = redisTemplate.opsForValue().get(lockKey); // redis里的时间
            if (currentValueStr != null && Long.parseLong(currentValueStr) < System.currentTimeMillis()) {
                //判断是否为空，不为空的情况下，如果被其他线程设置了值，则第二个条件判断是过不去的
                // lock is expired
                final String oldValueStr = redisTemplate.opsForValue().get(lockKey);
    ...
    public synchronized void unlock() {
        ...
        if (myExpires.equals(redisTemplate.opsForValue().get(lockKey))) {
            if (locked) {
